...
- **feat(validation-observability)**: 为每个实际执行用例增加 queue/setup/execute/persist 四段耗时与确定性 Top5，summary 暴露整体 `cache_hit_ratio` 和 Receipt miss 原因，fresh-clone 逐步暴露 `elapsed_ms`；新增只读 `validation explain`，精确命中时返回当前 fingerprint，miss 时才按 Receipt 文件 mtime 选择最新同 profile 完整性校验参考并逐字段对比，保持 `validation check` O(1) 精确路径不扫描。首轮八场景外部墙钟为 cold/warm Full `190962/424 ms`、one-go `198420 ms`、docs-only `426600 ms`、lifecycle-change `428270 ms`、cold/warm Release `490689/453 ms`、standalone fresh-clone Release `56633 ms`；docs-only explain 只指认 `subjectTreeOID`，但仍触发 425435 ms 的整份 Full 报告执行，作为 0017 立项证据；standalone clone 占 40991/56496 ms，作为 0018 立项证据。 / Adds per-case phase timing, deterministic Top5 and cache/miss summaries, step-level fresh-clone timing, and a read-only field-level validation explanation; records the first fixed eight-scenario baseline and preserves the exact Receipt check fast path.

- **ci(toolchain-evidence)**: feature 分支 workflow_dispatch #29798985041 实测 setup-go 为 `Setup go version spec 1.26.5`，且随后输出 `go version go1.26.5 windows/amd64`；该分支运行只用于 TODO 0015 工具链实证，不计入 ADR 0007 §5 所要求的 main `release-gate` 三次连续绿灯。 / Records the feature-branch CI proof for Go 1.26.5 and explicitly excludes this dispatch from the three consecutive main release-gate runs required by ADR 0007 section 5.

- **build(toolchain)**: 保留 `go 1.22` 最低语言兼容承诺，并以 `toolchain go1.26.5` 修复 GO-2026-5856；Full/Release 新增固定版本 Staticcheck/govulncheck，清零存量静态分析告警，四个 Action 全部钉 full SHA。远端实测 setup-go 的 `go-version-file: go.mod` 只解析到 `go 1.22`，故 CI 三处改为显式 `go-version: '1.26.5'`；已知欠账为“版本单源未达成，退化为双处维护”。有效 Go 工具链由 go1.26.4 变为 go1.26.5，因此旧 Receipt 全量 MISS 是 `toolchainDigest` 身份输入变化导致的正确失效，不是回归。 / Keeps the Go 1.22 language floor while selecting go1.26.5 for the security fix, adds pinned analysis and vulnerability gates, pins Actions, records the setup-go single-source limitation and explicit CI version fallback, and intentionally invalidates old Receipts through the changed effective-toolchain identity.
