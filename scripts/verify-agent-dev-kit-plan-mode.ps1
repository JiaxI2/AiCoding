...
function Test-ZhFirstLanguagePolicy([string]$Root) {
  $targets = @(
    ...
    "prompts/apply-devkit-plan-mode-overlay.prompt.md",
    "patches/aicoding-agent-dev-kit.SKILL.insert.md",
    ...
    for ($i = 0; $i -lt $lines.Count; $i++) {
      ...
      foreach ($pattern in $patterns) {
        if ($line.Contains($pattern)) {
          $severity = if ($rel -like "scripts/*.ps1" -or $rel -like "scripts/hooks/*.ps1") { "error" } else { "warning" }
...
try {
  ...
  $requiredFiles = @(
    ...
    "patches/aicoding-agent-dev-kit.SKILL.insert.md",
    "scripts/new-agent-plan-mode-session.ps1",
    "scripts/confirm-agent-decision.ps1",
    "scripts/verify-agent-dev-kit-plan-mode.ps1",
    "scripts/hooks/aef/plan-mode-gate.ps1",
    "scripts/hooks/aef/spec-artifact-gate.ps1"
  )
  ...
  Add-Check "decision.pending" ((-not $hasPendingDecision) -or $AllowPendingDecision) "检测到 spec/NEEDS_USER_DECISION.md，用户尚未选择技术路线，禁止继续实现。" @{ exists=$hasPendingDecision }
  ...
  if ($sensitive.Count -gt 0) {
    ...
    Add-Check "implementation.plan" $hasPlan "架构敏感变更需要 spec/IMPLEMENTATION_PLAN.md。" @{ approved=$hasApprovedPlan }
    Add-Check "implementation.tasks" $hasTasks "架构敏感变更需要 spec/TASKS.md。"
    Add-Check "implementation.traceability" $hasTrace "架构敏感变更需要 spec/TRACEABILITY.md。"
  } else {
