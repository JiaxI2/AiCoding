...
| AiCoding Agent Dev Kit | `aicoding-agent-kit` 需求澄清、方案矩阵、Spec/TDD、顺序上下文加载和进度监控 / clarify, option matrix, Spec/TDD, sequential loading, progress | [本地 Agent Kit](#local-agent-kits) |
...
- AiCoding Agent Dev Kit：`aicoding-agent-dev-kit`，来源为 `dist/aicoding-agent-dev-kit/plugins/AiCodingAgentDevKit`，提供 `aicoding-agent-kit`，用于需求澄清、技术方案矩阵、Spec Pack、TDD 计划、顺序上下文加载、轻量决策记忆、Hook bridge 和 MVP 进度监控。
