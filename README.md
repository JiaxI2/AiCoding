...
| Go 快路径 / Go fast path | Go 1.22+ 构建 `bin/aicoding.exe`，接管 Git hook、Smoke 检查、治理 lint 和性能探针 / Go 1.22+ builds `bin/aicoding.exe` for hooks, Smoke checks, governance lint, and perf probes | [Fast Path V1](#fast-path-v1) |
...
| Codex Agent PowerShell Skill Kit | PS7 AST、安全改写计划、Regex Optimization Gate、Go fast-path regex lint 和 PSScriptAnalyzer gate / PS7 AST, safe rewrite plan, Regex Optimization Gate, Go fast-path regex lint, and PSScriptAnalyzer gates | [本地 Agent Kit](#local-agent-kits) |
...
- Codex Agent PowerShell Skill Kit：`codex-agent-powershell-skill-kit`，来源为 `dist/codex-agent-powershell-skill-kit/plugins/AiCodingPowerShellSkillKit`，提供 repo-scoped PowerShell 7 guard、AST parser gate、安全改写计划、Regex Optimization Gate、Go fast-path regex lint、PSScriptAnalyzer gate 和 Agent 验证入口。当前 AiCoding 配置版本为 v1.3.0。
