...
`resultsDigest`，任何缺失、篡改、脏主体或 start/end identity 漂移都 fail-closed。第二期默认
保持 `--reuse off`；只有 main 远端 release-gate 连续 3 次 seed/audit 绿灯并由独立提交引用
run URL，才允许晋级 `auto`。pre-push 只检查受 policy 治理 ref 的 exact local_oid Receipt；
