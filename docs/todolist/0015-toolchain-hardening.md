...
3. **go 版本对齐**：因 GO-2026-5856 安全修复，go.mod 增 `toolchain go1.26.5` 行；CI 曾尝试三处
   `go-version-file: 'go.mod'`。远端 workflow_dispatch #29798713550 同时输出
   `Setup go version spec 1.22` 与 `go version go1.26.5 windows/amd64`：前者证明 setup-go 只用
   `go 1.22` 选择预装/缓存版本，后者证明 Go 命令正确读取 `toolchain go1.26.5`，在运行时自动
   下载并切换到 1.26.5。问题不是“CI 跑在 1.22”，而是 setup-go 未命中 1.26.5 缓存，后续
   每次运行都可能付出工具链下载成本。因此三处改为显式 `go-version: '1.26.5'`；
   workflow_dispatch #29798985041 随后出现 `Setup go version spec 1.26.5`、`Found in cache` 与
   `go version go1.26.5 windows/amd64`。**已知欠账仍是版本选择退化为 go.mod/CI 双处维护；
   这是消除运行时下载的缓存预取欠账，不是 toolchain 指令失效或有效 Go 版本错误。**
