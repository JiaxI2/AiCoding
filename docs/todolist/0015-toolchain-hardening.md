...
Status: Planned
Verify: bin/aicoding.exe test --profile Full --json 含 lint 节点全绿；CI 与本地 go 版本由 go.mod toolchain 单源决定
...
| 3 | **CI 与本地 go 版本漂移** | CI 三处硬编码 `go-version: '1.22'`，本地 go1.26.4，go.mod 无 toolchain 行 | toolchain digest 已入 Receipt 身份——**本地与 CI 的 Receipt 互不可比**，release-gate 三绿证据的可信度打折 |
...
3. **go 版本单源**：go.mod 增 `toolchain go1.26.4` 行；CI 三处改
   `go-version-file: 'go.mod'`。此后升 Go 版本只改 go.mod 一处，
   toolchain digest / CI / 本地自动一致。
...
grep -n "go-version" .github/workflows/aicoding-ci.yml   # 只剩 go-version-file
...
通过判据：staticcheck 负例被抓（贴输出）；govulncheck 在 Full 中执行；CI 工作流无硬编码
go 版本；四个 action 均为 SHA；toolchain 变化正确导致旧 Receipt 失效（这是特性不是回归，
CHANGELOG 说明）。
