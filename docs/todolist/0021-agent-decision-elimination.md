...
```go
// internal/report/result.go —— category/retryable 必需，成功也显式输出 none/false
Category  Category `json:"category"`  // 有限枚举，见下
...
- `report.Result` 只扩展既有 `config/schemas/cli-report.schema.json`：`category` 与
  `retryable` 对成功/失败均必需，失败由统一出口补齐 `nextAction`；非法、空缺或矛盾组合
  fail-closed 为 `internal/false`。封闭枚举与 schema 对齐测试通过。
- `change verify` 只复用 `gitx.StatusSnapshot`、Plan Mode pattern、
  `validationevidence` 和唯一 `testengine`。首次实现的 Receipt 路径因重复执行两次
  `git status` 得到 5 次墙钟中位数 `653.388 ms`，未达标；修正为首次快照直接构造
  HEAD/INDEX/dirty Subject 后，staged 命中中位数为 `381.694 ms`，无参数 clean HEAD 命中
  中位数为 `331.665 ms`，两组均 5/5 `executionMode=receipt-hit`、`executedCases=0`。
- 真实 docs-only INDEX（`CHANGELOG.md` + `docs/change-verify-probe.md`）选择 Smoke：
  67 项中 45 PASS、22 SKIP、0 WARN/FAIL；报告保存在忽略目录
  `test-results/0021-change-verify-20260722/probe-v2/aicoding-change-verify-20260722-015509.684964900/`。
- 真实 Go INDEX（`CHANGELOG.md` + `internal/plan/change_verify_probe_test.go`）选择 Full：
  67 项中 63 PASS、4 SKIP、0 WARN/FAIL；报告保存在忽略目录
  `test-results/0021-change-verify-20260722/probe-v2/aicoding-change-verify-20260722-015841.202545600/`。
- 真实失败输出：未知 Kit 返回 `usage/false` 与 `aicoding kit list --json`；Release Receipt
  miss 返回 `evidence-missing/false` 与 `aicoding test --profile Release --reuse off --json`；
  staged 混入 untracked 文件在 `changes.detect` 以 `validation/false` 和
  `git status --short` 拒绝。
- 第一次 docs 探针因临时 worktree 未初始化递归子模块且未暂存 CHANGELOG，Smoke 正确
  失败 5 项；失败和修复后 PASS 报告均保留在
  `test-results/0021-change-verify-20260722/docs-probe-v1/`，未通过放宽门禁获得绿灯。
- `go test ./internal/report/... ./internal/cli/... ./internal/testengine/...`、`go vet`、
  Staticcheck v0.7.0、DocSync、Plan check、dependency governance 与 `git diff --check`
  全部通过；`internal/` 未新增 workflow/engine/scheduler 包，typed catalog 未新增 `loop`。
...
通过判据：
1. **全仓仍只有一个 report schema**（不存在 `node-result.schema.json` 或 `NodeResult` 类型；
   宽泛的 `node-result` 文本只命中 TODO 0017 既有私有节点 Receipt 的 snapshot/fixture 标签）。
