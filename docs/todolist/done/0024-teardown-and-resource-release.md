...
> **staging 导入时的快照答案：没有，而且已经漏了 79 个目录 / 27 MB。**
...
该数字是导入计划时的活体 `%TEMP%` 快照，不是稳定 fixture。实现验收时重新按严格小写
`Ordinal` 前缀计数，最终清理前为 77 个 / 3,645,523 bytes；另有 1 个大小写不匹配的
`AiCoding-ppt-mcp-editable-check-019f6fa3`，被明确排除并保留。删除判据采用最终 dry-run
清单，而不是沿用 staging 估算。
...
- 创建临时目录时**追加一行**（`internal/platform` 加一个帮助函数统一命名 + 登记）；
  实现时扫描全部非测试 Go 调用点，当前仓库外部 `%TEMP%/aicoding-*` 的生产创建者只有
  `freshclone.go`，因此只迁移这一处；validation store 的仓内 `.tmp-report-*` 和只读
  CodingKit 子模块中的 `cstylekit-*` 不属于本 scope；
...
| pwsh / go 子进程 | 现有调用点混用 `exec.Command` / `exec.CommandContext`，本项无统一 kill 权威 | ⚠️ 只增加孤儿只读诊断；不借收尾项扩张为全仓取消语义重构 |
...
| 验证 | 实际结果 |
|---|---|
| 组合单测 | `go test -v ./internal/platform ./internal/cache ./internal/kit ./internal/repohealth ./internal/cli` 全部通过。 |
| 历史孤儿 dry-run | `plannedCount=77`、`plannedBytes=3645523`；77 个目标全部是系统 temp 的直接子目录且为严格小写 `aicoding-*`，`invalidTargets=[]`、`missingFromPlan=[]`、`unexpectedPlan=[]`。 |
| 显式收养回收 | `cache clean --scope temp --adopt --json` 返回 `removedCount=77`、`freedBytes=3645523`；随后严格小写残留为 0，大小写错误的保护目录仍存在。 |
| fresh-clone 成功路径 | Smoke 成功，ledger outcome 为 `created → releasing → released`，`keptTemp=false`，返回的 temp 路径已不存在。 |
| fresh-clone 失败路径 | `TestFreshCloneFailureRetainsRegisteredEvidence` 验证现场存在、`keptTemp=true` 且 ledger 最后一条为 `failed`；fixture 在断言后由测试清理。 |
| 保留与边界负例 | 5 个旧失败现场只计划删除 2 个并保留最近 3 个；24h 内和 `investigating` 均保留。默认只删当前 repoRoot，`--all-repos` 后才删其它 worktree；非小写前缀、work attempts 与 plan state 全部未触碰。 |
| bloat | 25 个隔离 temp fixture 触发 `temporary resources use 25 entries ... cache clean --scope temp --dry-run` warning。 |
| 进程诊断 | 实机 `doctor --all` 中 `doctor.orphan-processes=PASS`、`supported=true`、`candidateCount=4`、`orphans=[]`、`killAttempted=false`。 |
| Full | 首次运行 59 PASS / 1 FAIL / 1 WARN / 3 SKIP，暴露 temp 目录在跨 package 并行测试中于 `ReadDir` 后被释放的真实竞态，以及 staticcheck S1016；增加确定性并发释放回归并修复后，原始 `go test ./...` 与 staticcheck 均通过，Full 复跑为 61 PASS / 0 FAIL / 0 WARN / 3 SKIP，`duration_ms=194157`。 |

本项实现、负例、真实历史孤儿回收、DocSync 与 Full 均已通过，状态翻为 Done。
