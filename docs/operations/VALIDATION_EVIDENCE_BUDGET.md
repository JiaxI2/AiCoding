...
- 测量日期：2026-07-21；
- 实现候选提交：`13eddf172e90c1e92ff181107b716cf20f026959`，Tree：
...
- worktree：干净 detached 候选树；测量前递归初始化三个 pinned submodule；预构建该候选树的
  `bin/aicoding.exe`；
...
- “cold/warm”只描述 Receipt：cold/变更场景完整执行，warm 使用 `--reuse auto` 命中；Go
  toolchain/module/build cache 均为暖缓存。每个场景是本机首轮单样本观测，不建立 SLA；
...
完整本机报告位于 ignored `test-results/0016-baseline-20260721/`。

### 9.2 八场景实测

| 场景 | 命令/变化 | 模式 | cache ratio | outer_ms | report_ms | 结论与最慢项 |
|---|---|---|---:|---:|---:|---|
| cold-full | `test Full --reuse off` | executed | 0 | **190962** | 189733 | 60 PASS；GO-002 82974 / GO-001 77072 ms |
| warm-full | 同 Tree，`test Full --reuse auto` | reused | 1 | **424** | 留存 189733 | 同 Receipt，60 PASS |
| one-go-file | 单 Go 注释，`test Full --reuse auto` | executed miss | 0 | **198420** | 197176 | GO-002 85803 / GO-001 81725 ms |
| docs-only | 单 README 变更，`test Full --reuse auto` | executed miss | 0 | **426600** | 425435 | GO-002 200126 / GO-001 196188 ms |
| lifecycle-change | kit registry JSON 空白，`test Full --reuse auto` | executed miss | 0 | **428270** | 427039 | GO-002 199697 / GO-001 199165 ms |
| cold-release | `test Release --reuse off` | executed | 0 | **490689** | 489468 | 63 PASS；GO-001 195696 / GO-002 195158 / FRESH-001 66956 ms |
| warm-release | 同 Tree，`test Release --reuse auto` | reused | 1 | **453** | 留存 489468 | 同 Receipt，63 PASS |
| fresh-clone-release | `fresh-clone --profile Release` | executed | n/a | **56633** | CLI 56496 | 5/5 steps PASS，均有 `elapsed_ms` |

所有四个完整执行的 Full 样本和 cold-release 中，带 timing 的用例均满足
`queue_ms + setup_ms + execute_ms + persist_ms == duration_ms`，不一致数均为 **0**。
Receipt miss 场景的 summary 均为 `cache_hit_ratio: 0` 且带
`VALIDATION_RECEIPT_MISS: no reusable Receipt exists`；warm 场景为 `1` 且无 invalid reason。
...
- 0017：docs-only 只改变 `subjectTreeOID`，却仍付出 `425435 ms` 整份 Full 报告执行；
  warm Full 同机只需 `424 ms`。这是分域失效值得继续设计的直接证据，但不预设节点边界。
