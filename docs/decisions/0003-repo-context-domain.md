...
按 [Primitive 宪法](../architecture/PRIMITIVE_CONSTITUTION.md) §12 逐项自评，附具体证据。
这也是后续新 Primitive/新领域 ADR 的自评范式。

**架构**
- 只有一个职责？是——每个动作单一职责（§5 表：status=新鲜度 / doctor=完整性 / verify=结构 / install|update=生成收敛 / uninstall=移除）。
- 可继续拆分？否——四段领域纯函数（scan/render/reconcile/affectedDomains）已最小。
- 能被直接复用？是——复用 `registry.NewSnapshot`、lifecycle 八动词、`report`、hook、`gitx`（§1 表）。
- 存在重复实现？否——与 kit/mcp 的 owned 形态不同；已标注"再出现第二个文本域则抽 `reconcile`"路标。
- 真的需要新 Primitive？是——InputKind 与 StateOwner 无处安放（§2），走扩展路径③ + ADR。

**性能**
- 有 Fast Path？是——`reconcile` 只写 diff、`Sync` 只涉受影响域、doctor/verify 零扫描。
- 无关扫描？无——只有需要事实的 install/update/status 各扫一次，uninstall/doctor/verify 零扫描（守卫测试 `TestRepoContextDoctorAndVerifyDoNotScanWhenNotInstalled`、`TestRepoContextUninstallReadsManifestWithoutScanning`）。
- 重复 IO / 重复计算？无——adapter 复用动作带回的 `factsDigest`，不再二次扫描（每动作至多一次）。
- 重复 Agent/工具调用？无——纯 Go、零 LLM、零外部进程（`Entrypoint: go-static`）。
- 最小 Context / 进一步降 Token？是——每域约 35 行 scoped context 按需加载，胜过单体索引。
- 进一步减文件读取 / Git 操作 / 进程启动？已到位——sync 一次 `git diff-tree`；生成物 gitignore 不触碰 git 对象；无子进程。
- 实测成本：`Scan` ~2ms/全仓、`reconcile` 收敛态 ~0.27ms（§5.1，`BenchmarkScan`/`BenchmarkReconcileNoOp`）。

**质量**
- 确定性？是——`Facts`/`Manifest`/生成物排序、无时间戳、无绝对路径，digest 恒等；耗时只进信封。
- 接口稳定？是——不加新动词/新顶层命令；`AdapterResult.elapsedMs` 为附加字段（兼容）。
- 最小输入/输出？是——动作签名仅 `(repo[, dryRun])`；输出标准 `report.Result` 信封。
- 独立测试/Benchmark？是——`internal/repocontext` 全套单测 + `BenchmarkScan`/`BenchmarkReconcileNoOp`。
- 自由组合？是——门禁（doctor --all/verify）与 post-commit hook 均由上层组合零扫描 Primitive 而成。
