...
alias。节点层只服务测试引擎内部复用，不改变整树 Receipt 的外部凭证地位。第 5 节的
`toolchainDigest.v2` 晋级门槛已经由三棵不同 main Tree 的远端 seed/audit 满足；ADR 0014
因此在独立提交中把默认值晋级为 `--reuse auto`。Plan Mode 与 Profile 继承继续明确排除。
...
以下门槛定义一次身份方案从 `--reuse off` 晋级为默认 `--reuse auto` 所需的证据：
...
Validation Evidence Budget §13 已记录 `toolchainDigest.v2` 在三棵不同 main Tree 上的
3/3 远端证据；ADR 0014 于 2026-07-23 完成独立晋级。其更严格、可判定的立即回滚条件向前
取代本段旧草案：任意一次 Tree 已变化却命中 Receipt，或任意一次真实失败被 Receipt 掩盖，
默认值立即退回 `off`。再次晋级仍须按本节重新累计新一轮 3/3。

## 6. Rollback

默认 `--reuse auto` 只在完整身份与报告完整性校验后命中；显式 `--reuse off` 仍是完整
TestCase 执行与默认值回滚路径。Context Gate 回滚可移除 `.githooks/pre-push`/policy 接线
而不删除任何证明。Git common-dir 下的
