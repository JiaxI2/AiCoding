...
pre-push exact-tree 门禁与 commit alias。仅完成定期 seed/audit CI 接线不等于已有远端绿灯证据，
因此默认保持 `--reuse off`；`auto` 在满足第 5 节晋级门槛前只允许显式启用。Plan Mode 与
Profile 继承继续明确排除。
...
Receipt/alias 是可再生证据，Receipt 可由 `validation clean` 删除，不属于版本化工作区。
Plan Mode 无需参与回滚。
