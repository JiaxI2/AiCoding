...
| 领域层（互相隔离的兄弟节点） | `internal/kit`、`internal/mcpcontrol`、`internal/repocontext`、`internal/capability` | 各自 owned state；capability 只读 registry 投影 | 按扩展路径新增 |
...
3. **领域互相隔离**：`kit`/`mcpcontrol`/`repocontext`/`capability` 互不 import；`capability` 只向下依赖 `registry`。
