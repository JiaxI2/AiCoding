...
| Governance | 强制拦截：不合规的改动进不来 | `.githooks/` + `aicoding hook`、governance 四门禁、docsync、`.clang-format`、`.github/` 模板 | [05](05-governance.md) |
...
| `governance lint\|dependencies\|layout\|reuse` | 四个治理门禁：提交规范、依赖方向、目录布局、复用证据。 |
...
| `cache status\|clean` | 观测四类已注册本地生成物，并按失败证据、Receipt/alias 与审计轨迹保护规则执行保留策略。 |
