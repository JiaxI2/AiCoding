...
| 4 体检 freshness | ✅ 已落地 | 挂入聚合 `doctor --all`（`doctor.repo-context`）与 `verify --profile`（`verify.repo-context`），并在唯一测试 Registry 登记 leaf gate（`RC-001` 结构验证 / `RC-002` 生成计划）。代码事实 digest vs 生成物记录 digest 对账：**完整性破坏（缺失/被篡改的 owned 文件）为 error，失败门禁**；**新鲜度漂移为 warning，呈现但不失败**（漂移是瞬时的、由 post-commit hook 自愈；未安装时全部空操作，fresh clone/CI 不受影响） | 上下文完整性破坏被拦截、漂移被呈现 | 完整性破坏报 error、漂移报 warning（`TestDoctorDetectsTampering`、`TestStatusReportsFreshThenDriftAfterCodeChange`） | 否 |
