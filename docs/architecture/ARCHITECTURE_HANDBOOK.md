...
**问：为什么不做一个统一 Core 简化调用？**
答：Git 用 40 年证明了反例。`blob/tree/commit/ref` 各自独立，所以 packfile 换了
十几版格式、传输协议升到 v2、gc 策略反复重写——快照语义一行没改。统一 Core 意味着
任何一处优化都要理解全部，验证半径永远是全量。

**问：为什么 adapter 是静态编译的，不做动态插件？**
答：动态加载让"本次运行执行了哪份代码"无法回答（10 号笔记的核心论证）。静态
catalog + digest 让每次运行可对账。可玩性来自组合稳定组件的自由，不来自身份不确定。

**问：为什么 plan 和 apply 共用同一领域路径（plan = action + dryRun）？**
答：两套路径必然漂移——plan 说能装、apply 装失败。Git 的 index 同理：`git add`
写入的就是 commit 将要用的 tree 条目，不是另一套预览格式。

**问：为什么 digest 有三个（catalog/input/plan）而不是一个？**
答：它们回答三个独立问题：用了哪组契约、对什么事实、执行什么意图。合并成一个就
无法定位漂移来源——如同 Git 区分 tree OID 和 commit OID。
