...
工作流知识的现存实例：`aicoding-upgrade-train`（受管组件升级列车，
preflight→plan→confirm→apply→verify 与分域 rollback）与
`aicoding-environment-rebuild`（环境重建，fresh-clone 证明→bootstrap→
install plan→确认→apply→全量验证），均经 Draft 门禁验证后进入
RepoLocal（`.agents/skills/`）。早期草稿 `aicoding-external-integration`
（集成决策 pin/fork-pin/adopt）已废弃且未曾进入运行时；其获取侧流程由
[契约冻结与获取/激活边界](FREEZE_AND_ACQUISITION_BOUNDARY.md) §3.2 固化，
集成决策工作流知识在真实集成场景出现时按 Draft 阶梯重建。
