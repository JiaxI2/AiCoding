...
三份 README 的 badge 必须完全一致，且每个 badge 标题必须以大写英文字母开头（由依赖治理门禁机器校验）。第三方明确版本链接官方 release/tag/版本文档；本地 Kit badge 链接本仓库 Kit 说明，并与对应 manifest 的 `version` 一致。README 正文不直接散落版本号。
