...
try {
  ...
  if ($Dev -or $Editable) {
    Write-Warning "Installing Agent Patch Kit in editable/dev mode. Do not delete this source directory."
    python -m pip install --force-reinstall -e .
  } else {
    Write-Host "Installing Agent Patch Kit in user mode: non-editable, self-contained wheel. Source zip/folder can be deleted after validation."
  ...
  Write-Host "Agent Patch Kit install complete. Non-editable installs do not depend on the extracted source directory."
}
