# Agent Patch Kit v0.2.2

## v0.2.2 install behavior

Default install is now non-editable and self-contained. After `apatch install doctor` reports `install_mode: non-editable / user mode` and `bundle_assets: OK`, the original zip and extracted source folder can be deleted. Use `-Dev` only when developing this kit.
...
Repair a broken v0.2.1 editable install:

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File scripts\repair-agent-patch-kit.ps1
```

See `docs/INSTALL_MODE.md` and `docs/CODEX_REINSTALL_PROMPT.md`.
