...
$result = [pscustomobject]@{
    ...
    Checks = @($checks | ForEach-Object { [pscustomobject]@{ Path = $_; Exists = (Test-Path -LiteralPath (Join-Path $repo $_)); Required = $true } }) + @($runtimeChecks | ForEach-Object { [pscustomobject]@{ Path = $_; Exists = (Test-Path -LiteralPath (Join-Path $repo $_)); Required = $true } })
}
